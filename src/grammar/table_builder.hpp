...
class TableBuilder {
public:
    void build(const FirstSet& first, const FollowSet& follow_sets);
    // Доступ к результатам (rvalue reference)
    std::vector<ItemSet>&& take_states()    { return std::move(states_); }
...
private:
    ...
    void add_reduce(size_t state_id, const FollowSet& follow_sets, Item& item);
